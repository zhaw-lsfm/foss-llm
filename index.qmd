...
format:
  revealjs:
    theme: dark
    css: [styles/base.css, styles/custom.css]
    slide-number: true
    transition: slide
...
# Open Source LLMs {background-image="images/table-setting-6859276_1280.jpg" class="blurred-10px shadow"}

[And why they matter]{.shadow}
...
```{r}
#| fig-cap: Daily hours spent on cooking / preparing meals vs. hours spent on the computer. Ratnaweera et al (2025), n = 1

library(ggplot2)

df <- data.frame(activity = c("Cooking", "Computer"), hours = c(1.25, 6.72))
...
:::{.notes}
- Sometimes products are plain bad, lack basic security features and the safety of userdata is no priority
...
::: {.notes}
- Sometimes Software versions are not supported anymore, as seen in the example of the End-of-Life for Windows 10.
...
- Especially, we don't need to understand all 3 layers (next slide)
...
## Fork in the Road {data-background-image="images/forest-6607631_1280.jpg" .center .shadow}
...
# OpenSource & LLM {background-image="images/books-2596809_1280.jpg" class="blurred-10px shadow"}


## {background-image="images/books-2596809_1280.jpg"}
...
::: {.incremental}
- Fully Open Source (fully reproducible)
- Open Weights + Code (auditable methodology, but not data)
- Open Weights (runnable, but not verifiable)
...
- Complete black box - you only get API access
- No insight into how it works, what data it was trained on, or how decisions are made
- You're entirely dependent on the vendor

**Open Weights (Llama 3.3, DeepSeek, Mistral, Qwen):**
- The trained model weights are released - you can download and run them locally
- BUT: Training code and training data typically NOT included
- You can USE it, but can't fully verify or reproduce how it was created
- This is where MOST "open" models sit today
- For practical teaching purposes: This is sufficient! You get local control.

**Open Weights + Code (Some research releases):**
- Weights + the methodology/code used for training
- You can understand and audit HOW it was trained
- BUT: Training data usually not included (licensing issues, competitive advantage)
- Example: Some academic lab releases
- Closer to true "open source" philosophy - transparent and auditable

**Fully Open Source (OLMo, BLOOM - rare!):**
- Weights + Training Code + Training Data - everything
- Fully reproducible and auditable
- This is the gold standard but extremely rare (training data is expensive, complex licensing)
- Most similar to traditional open source software
...
# Practical Demo {background-image="images/magic-note-8836935_1280.jpg" .shadow .blurred-5px}

## {background-image="images/magic-note-8836935_1280.jpg"}

## Step 1: Get a Runtime Environment

You need software that can **run** the model on your machine

::: {.incremental}
- Downloads the model files
- Manages memory and processing
- Provides an interface (API, chat, etc.)
:::
...
**Ollama** (ollama.ai) - Recommended for beginners  

- Command-line tool, very simple
- Works on Mac, Linux, Windows
- Easy model management: `ollama pull deepseek-r1`
- Built-in API server
- Free and open source

**LM Studio** (lmstudio.ai)

- Graphical interface, very user-friendly
...
- Limited model selection but curated
- Free and open source

**Jan.ai**

- Desktop app, modern interface
- Local-first philosophy
- Good privacy focus
- Free and open source

**For this workshop, we'll focus on Ollama** - it's the most flexible and widely used, but any of these will work for getting started.
...
::: {.incremental}
- **Size** (1GB to 100GB+)
- **Capability** (general chat, coding, reasoning)
- **Language** (multilingual vs English-focused)
:::

::: {.fragment}
Start with a **small, general-purpose model**
:::
...
- **DeepSeek-R1:1.5B or 7B** - Excellent reasoning, good for teaching tasks, reasonable size
- **Llama 3.2 3B** - Fast, capable, widely supported
- **Qwen 2.5 7B** - Strong multilingual, good for international contexts
- **Phi-4** - Microsoft's efficient small model, surprisingly capable

**Size considerations:**
- **1-3B parameters:** Fast, runs on most laptops, good for basic tasks (2-6GB RAM needed)
- **7-8B parameters:** Sweet spot for teaching - good quality, reasonable speed (8-16GB RAM)
- **13B+ parameters:** Better quality, but slower and needs more RAM (16GB+ minimum)
...
- Search for "deepseek" or "llama"
- Look for models marked with your RAM capacity
- Click download

**Key point:** You can always download more models later. Start small, test it out, then try larger models if needed. Models are just files - you can delete them if you don't like them.
...
**With LM Studio (GUI):**
1. Select your downloaded model from the left sidebar
2. Click "Load Model"
3. Type your prompt in the chat interface
4. Hit enter

**With GPT4All or Jan.ai:**
- Similar - select model, type in chat box, get response

**First prompts to try:**
- "Generate 5 practice questions about [your subject]"
- "Summarize this text in simple language: [paste text]"
- "Give feedback on this student paragraph: [paste text]"
- "Explain [concept] at a high school level"

**What's happening behind the scenes:**
- The model file is loaded into RAM
- Your text is converted to numbers (tokens)
- The model predicts the next token, over and over
- Results are converted back to text
- All happening **on your machine**, no internet needed (after download)

**Key insight:** Once you get to this point, you realize it's not magic - it's just software running locally. You have full control.
